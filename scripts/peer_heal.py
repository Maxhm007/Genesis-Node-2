#!/usr/bin/env python3
# simulation-consume: node-2-quorum-20260818-0237
...
def main():
    node=os.getenv('NODE_ID','genesis-node-2'); own=os.getenv('GITHUB_REPOSITORY') or os.getenv('NODE_REPO'); peers=[p.strip() for p in os.getenv('PEER_REPOS','').split(',') if p.strip()]
    for peer in peers:
