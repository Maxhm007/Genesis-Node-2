...
def valid_json(path):
    try:
        json.loads(pathlib.Path(path).read_text(encoding='utf-8')); return True
    except Exception: return False

def restore(path):
    p=subprocess.run(['git','show',f'HEAD^:{path}'],text=True,capture_output=True)
    if p.returncode: return False
    pathlib.Path(path).write_text(p.stdout,encoding='utf-8')
    return valid_json(path)

def main():
    bad=[p for p in CRITICAL if not valid_json(p)]
    if not bad:
        print('Node health OK'); return 0
    print('Local self-heal required:', ', '.join(bad))
    failed=[p for p in bad if not restore(p)]
    if failed:
        print('Unable to restore:', ', '.join(failed), file=sys.stderr); return 1
